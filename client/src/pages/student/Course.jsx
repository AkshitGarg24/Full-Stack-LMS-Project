...
const Course = () => {
  ...
  useEffect(() => {
    ...
    if (x) {
      if (x != course) {
        setCourse(x)
        setRatings(calculateCourseRatings(x))
        setPreview(false)
      }
    ...
    course ?
      <div className='lg:flex-row flex flex-col-reverse sm:px-20 px-10 lg:py-30 py-20 gap-10 bg-gradient-to-b from-cyan-100/70'>
        ...
        <div className='lg:w-200 w-full border-1 rounded-2xl border-gray-300 shadow-lg pb-10'>
          {
            preview.preview && pathname.includes('/course') ? <YouTube
          ...
          <div className='px-10 py-5 flex flex-col gap-6'>
            ...
            <div className='w-full px-5'>
              {preview.preview && pathname.includes('/course') ? <button onClick={() => setPreview({ preview: false })} className='bg-red-500 text-white p-5 rounded-2xl w-full cursor-pointer hover:bg-red-600'>Stop Preview</button> : <button className='bg-blue-500 text-white p-5 rounded-2xl w-full cursor-pointer hover:bg-blue-600'>Enroll Now</button>}
